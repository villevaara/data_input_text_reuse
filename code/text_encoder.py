...
class TextEncoder:
    ...
    def make_mapping(self, language):
        if language.lower() == "fin":
            return {"ä": "W", "v": "V", "a": "B", "i": "C", "s": "H", "m": "N",
                    "d": "Z", "h": "S", "e": "G", "u": "K", "f": "W", "w": "Y",
                    "o": "I", "y": "T", "l": "F", "j": "R", "n": "E", "r": "P",
                    "k": "M", "p": "Q", "t": "D", "z": "X"}

        elif language.lower() == "eng":
            return {"u": "E", "n": "W", "t": "R", "b": "V", "d": "C", "g": "Z",
                    "f": "N", "s": "F", "m": "K", "w": "M", "i": "G", "p": "I",
                    "o": "Y", "e": "H", "h": "S", "r": "T", "l": "A", "k": "X",
                    "c": "B", "a": "W", "y": "Q", "j": "D", "v": "P"}

        elif language.lower() == "eng_space":
            return {"o": "Y", "a": "W", "t": "R", "f": "N", "w": "M", "h": "S",
                    "l": "A", "v": "P", "y": "Q", " ": "D", "e": "H", "r": "T",
                    "i": "G", "s": "F", "p": "I", "c": "B", "b": "V", "m": "K",
                    "d": "C", "g": "Z", "u": "E", ".": "X", "n": "W"}
    ...
    def encode_text(self, text, preprocess=False):
        ...
        encoded_text = re.sub("[^" + "".join(list(self.mapping.keys())) + "]",
                              "", encoded_text, flags=re.DOTALL)
        for char, amino in self.mapping.items():
            encoded_text = re.sub(u"["+char+"]", amino, encoded_text,
                                  flags=re.DOTALL)
        return encoded_text

    # Masks all character to X that don't have an equivalent in the mapping
    def encode_mask(self, text):
        return re.sub("[^" + "".join(list(self.mapping.keys())) + "]", "X",
                      text.lower(), flags=re.DOTALL)

    # reverse protein mapping, not fully decoded,
    # just using the reverse map here
